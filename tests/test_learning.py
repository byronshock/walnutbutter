...
def test_teacher_validates_tracks_and_reports():
    ...
    assert grid.centred and all(n.centred and n.traced for n in grid.all_neurons())  # §8.6, the single-spike rule
...
def test_reinforce_clips_to_the_grid_weight_range():
    ...
    grid.set_delta(ESCAPE_DELTA)  # §8.3: the reinforce rule refuses where the threshold decides
    run_epoch(grid, verbose=False, rng=random.Random(2))
    before = {c.id: c.weight for c in grid.connections.values()}
    reinforce(grid, advantage=-1.0, lr=50.0)  # a huge push: every weight it moves should land on a bound, not past one
    moved = [c for c in grid.connections.values() if c.weight != before[c.id]]
    assert moved
    # §8.4's entries carry both signs, so one advantage drives some synapses to the floor and others to the ceiling
    assert any(c.weight == 0.001 for c in moved) and any(c.weight == 1.0 for c in moved)
    assert all(0.001 <= c.weight <= 1.0 for c in grid.connections.values())  # §0.5: the weight range is the one clip
