...
def arms(results):
    ...
    for r in results:
        groups[(r["rule"], r["drive"], r["read"], r.get("trace", "all"), r["h0"], r["lr"])].append(r)
    rows = {}
...
def best_per_h0(rows):
    best = {}
    for (rule, drive, read, trace, h0, lr), v in rows.items():
        k = (rule, drive, read, trace, h0)
...
def main():
    ...
    for name in sorted(os.listdir(d)):
        if name.startswith("toy2_") and name.endswith(".json"):
            results += load(d, name)
    rows = arms(results)
    best = best_per_h0(rows)
    print("| rule | drive | read | trace | h0 | best LR | accuracy | corr with d | whispers on / off | spikes/epoch | stuck | flipped | to 0.8 |")
    print("|---|---|---|---|---|---|---|---|---|---|---|---|---|")
    for k in sorted(best, key=lambda k: (k[0], k[1], k[2], k[3], -k[4])):
        lr, v = best[k]
        rule, drive, read, trace, h0 = k
        print(f"| {rule} | {drive} | {read} | {trace} | {h0:g} | {lr:g} | {v['acc']:.3f} ± {v['acc_sd']:.3f} | {v['corr']:+.2f} | "
              f"{v['won']:.2f} / {v['woff']:.2f} | {v['spikes']:.1f} | {v['stuck']:.1f} | {v['flipped']:.2f} | {v['t80'] or '—'} |")
    print("\nevery arm:\n")
    print("| rule | drive | read | trace | h0 | LR | accuracy | corr | flipped |")
    print("|---|---|---|---|---|---|---|---|---|")
    for k in sorted(rows, key=lambda k: (k[0], k[1], k[2], k[3], -k[4], k[5])):
        v = rows[k]
        print(f"| {k[0]} | {k[1]} | {k[2]} | {k[3]} | {k[4]:g} | {k[5]:g} | {v['acc']:.3f} ± {v['acc_sd']:.3f} | {v['corr']:+.2f} | {v['flipped']:.2f} |")
    if not fig:
        return
    import matplotlib
    matplotlib.use("Agg")
    ...
    series = sorted({(k[0], k[1], k[2], k[3]) for k in best})
    styles = {("local", "forced", "spikes", "all"): ("tab:orange", "-", "s"),
              ("local", "potential", "spikes", "all"): ("tab:red", "-", "^"),
              ("exact", "forced", "transmissions", "all"): ("tab:blue", "-", "o"),
              ("exact", "forced", "transmissions", "ventured"): ("tab:blue", "--", "o"),
              ("exact", "potential", "transmissions", "all"): ("tab:purple", "-", "v"),
              ("exact", "potential", "transmissions", "ventured"): ("tab:purple", "--", "v"),
              ("none", "forced", "spikes", "all"): ("gray", ":", "x"),
              ("none", "potential", "spikes", "all"): ("gray", "--", "x")}
    for s in series:
        h0s = sorted({k[4] for k in best if k[:4] == s})
        ...
        label = (f"{s[0]} rule, {s[1]} drive" + (", read on transmissions" if s[2] == "transmissions" else "")
                 + (", ventured trace" if s[3] == "ventured" else ""))
