...
Two hazard families, both h0 at rest and 1 spike per hop at threshold, both
scaled by sqrt(60/N) as the file scales every hazard (6.6); h0 is the hazard
at rest, the quantity Byron's third answer leaves "to be determined through
rigorous investigation", and it is what this toy sweeps:

  --hazard loglinear   h(u) = h0 ** (1 - u). Today's exponential hazard is
                       this family with h0 = exp(-1/Delta), clipped at zero
                       potential and cut off at threshold. h0 = 0 leaves no
                       speculation anywhere below threshold.
  --hazard linear      h(u) = h0 + (1 - h0) * u: Byron's statement read
                       literally, "monotonically increasing as u". h0 = 0 is
                       a synapse that never speculates at rest and speculates
                       in proportion to its source's potential above it.

The drive is a rate, as the file's is: --rate spikes per on-input per wave,
so a longer epoch carries proportionally more input spikes (0.15 gives the
first toy's three spikes in twenty waves).
...
RATE = 0.15              # spikes an on-input makes per wave: three in the first toy's twenty waves
...
def run(rule, B, K, waves, drive, h0, lr, epochs, seed, read="spikes", trace="all",
        hazard_family="loglinear", rate=RATE, refractory=2, checkpoints=20):
    spikes_per_on = max(1, int(round(rate * waves)))
    ...
    for ep in range(1, epochs + 1):
        ...
        for w in range(waves):
            ...
            m_in = kappa * hazard(hazard_family, h0, u)
            P_in = -np.expm1(-m_in)
            ...
            if read == "transmissions":
                ...
                m_o = kappa * hazard(hazard_family, h0, u_o)
                P_o = -np.expm1(-m_o)
    ...
    return dict(rule=rule, B=B, K=K, fan_in=d, waves=waves, drive=drive, h0=h0, lr=lr, read=read,
                trace=trace, hazard=hazard_family, rate=rate, spikes_per_on=spikes_per_on,
...
def main():
    ...
    ap.add_argument("--hazard", default="loglinear", choices=["loglinear", "linear"])
    ap.add_argument("--rate", type=float, default=RATE)
    ap.add_argument("--B", type=int, nargs="+", default=[32])
    ap.add_argument("--K", type=int, default=5)
    ap.add_argument("--waves", type=int, nargs="+", default=[20])
    ...
    args = ap.parse_args()
    jobs = [dict(rule=rule, B=B, K=args.K, waves=waves, drive=drive, h0=h0, lr=lr,
                 epochs=args.epochs, seed=seed, read=args.read, trace=args.trace,
                 hazard_family=args.hazard, rate=args.rate)
            for rule, drive, B, waves, h0, lr, seed
            in product(args.rule, args.drive, args.B, args.waves, args.h0, args.lr, args.seeds)]
    ...
    for r in results:
        h = r["hist"][-1]
        print(f"{r['rule']:6s} {r['drive']:9s} {r['hazard']:9s} read {r['read']:13s} trace {r['trace']:8s} waves {r['waves']:3d} h0 {r['h0']:<6g} "
