...
options = set(sys.argv[6].split(",")) if len(sys.argv) > 6 else set()
suppress = "suppress" in options
nofloor = "nofloor" in options
...
if nofloor:  # the counterfactual: the outputs' floor taken away, inhibition no longer saturates
    for n in outs:
        n.minimum_potential = -1e9
...
print(f"{label}: epoch offset {offset}, tau {Neuron.tau}, hop {hop}, drive {g.drive}, exploration {g.exploration}, "
      ...
      f"escape_scale {g.escape_scale:.5g}, synapse_scale(in0) {ins[0].synapse_scale:.5g}, options {sorted(options)}", flush=True)
...
summary = {"label": label, "kind": kind, "drive": g.drive, "source": source, "epochs": epochs, "suppress": suppress, "nofloor": nofloor,
